...
void ClientConnection::ProcessRecv(char *buf, int len, sockaddr_in *addr) {
	...
	if (type == ALVR_PACKET_TYPE_TRACKING_INFO && len >= sizeof(TrackingInfo)) {
		...
		double pitch;
		double fov;
		quat[0]=m_TrackingInfo.HeadPose_Pose_Orientation.x;
		quat[1]=m_TrackingInfo.HeadPose_Pose_Orientation.y;
		quat[2]=m_TrackingInfo.HeadPose_Pose_Orientation.z;
		quat[3]=m_TrackingInfo.HeadPose_Pose_Orientation.w;
		m_angle->Quaternion_to_pitch(quat, pitch);
...
// angle_diff::angle_diff():angle_now(0), angle_past(0){}
// angle_diff::~angle_diff(){}

// void angle_diff::set_angle_now(float angle_now)
// {
// 	this->angle_now = angle_now;
// }

// void set_angle_past(float angle_past)
// {
// 	this->angle_past = angle_past;
// }

// float get_fov_ration()
// {
// 	this->angle_d = this->angle_now - this->angle_past;
// 	float fov;
// 	fov = 1 - 0.3*(this->angle_d);
// 	return fov;
// }
angle::angle():angle_now(0), angle_past(0)
{
...
void angle::set_angle_now(double angle_now)
{
	this->angle_now = angle_now;
}

void angle::set_angle_past(double angle_past)
{
	this->angle_past = angle_past;
}

double angle::get_fov_ratio()
{
	this->angle_d = this->angle_now - this->angle_past;
	double fov;
	fov = 1 - 0.3*(this->angle_d);
