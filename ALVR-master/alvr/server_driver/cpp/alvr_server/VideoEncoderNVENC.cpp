...
double VideoEncoderNVENC::Fov_ratio()
{
	
	
	if(m_Listener){
		m_Listener->GetTrackingInfo(this->m_TrackingInfo);
		double tmp;
		...
		m_angle->Quaternion_to_pitch(quat, yaw_pitch_roll);
		
		// m_angle->set_angle_now(yaw_pitch_roll[0]);

		// for(int i=0;i<3;++i)
		// {
		angle_now=yaw_pitch_roll[1];
		angle_d=abs(angle_now-angle_past);
		angle_past = angle_now;
		// tmp =1-0.5*(angle_d*72);
		// }
		// double big=tmp[0];
		// int idx=0;
		// for(int i=0;i<3;++i)
		...
		// }
		// fout << "yaw: "<<tmp[0]<<endl;
		// fout<<"pitch: "<<tmp[1]<<endl;
		// fout<<"roll: "<<tmp[2]<<endl;
		// fout<<"======="<<endl;

		// tmp = m_angle ->get_diff();
		// m_angle-> get_fov_ration(tmp);
		tmp =1-0.4*(angle_d*72);
